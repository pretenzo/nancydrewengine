...
// ── Changelog ──────────────────────────────────────────────────────────────
//
// ## Ambient sound — case-insensitive loading (setAmbient / tryAmbientVariant)
//   Audio filenames use mixed case (e.g. Feardang.wav, Aunthous.wav,
//   Mystlght.wav) but scene data references them in lowercase (feardang,
//   aunthous, mystlght).  setAmbient now tries multiple case variants
//   (as-is → Title Case → UPPER → lower) sequentially until one loads.
//   A lightweight key (ambientKey) tracks the current ambient by lowercase
//   name so the same track isn't restarted on scene transitions, and stale
//   loads are cancelled if the scene changes while a variant is loading.
//
// ## Ambient sound — skip on conversation scenes (loadScene)
//   CONVERSATION_VIDEO scenes in the game data carry placeholder ambient
//   values (e.g. "aunthous" on 53% of conversation scenes, "mystlght" on
//   26%) that would override the location's real ambient.  The original
//   engine keeps the previous location's ambient during conversations.
//   loadScene now detects scenes containing CONVERSATION_VIDEO actions and
//   skips the setAmbient call for them, preserving the ambient from the
//   prior navigation scene.
//
// ## Narration video playback (playNarrationVideo)
//   CONVERSATION_VIDEO actions with empty npc_text (e.g. "<i><n><o>" which
//   strips to "") are not text conversations — they are narration sequences
//   that play an AVF video overlay on the background.  Affected scenes:
//     S62   (intro narration → S601 Aunt Eloise's house)
//     S2071 (epil4 → S2072)
//     S2072 (epil5 → S2073)
//     S2073 (epil6 → WIN_GAME, no continuation_scene)
//   playNarrationVideo loads all AVF frames for the node_id, draws them at
//   the screen rect from coords (x2,y2)→(x3,y3) at ~10fps, and blocks
//   input via movieActive.  The narration handler in loadScene then applies
//   tail_entries flags and either navigates to continuation_scene or falls
//   through to check for WIN_GAME/LOSE_GAME actions (S2073 case).
//
// ## PLAY_DIGI_SOUND — nav_on_end support
//   PLAY_DIGI_SOUND actions can embed a target scene at blob offset 0x1e
//   (parsed as nav_on_end in nd1-scene.py).  When the sound finishes, the
//   engine navigates to that scene.  This drives the intro flow (S60's
//   letter-reading sound navigates to S61 or S62 depending on difficulty)
//   and 109 other transitions (books, doors, drawers).  Navigation uses a
//   non-blocking .then() with a stale-scene guard so hotspots still work
//   (e.g. player can click to skip S60's intro).
//
// ## PLAY_SECONDARY_MOVIE — data-driven positioning (getMovieRect)
//   Movie clip screen positions are stored in per-frame entries at blob
//   offset 0xD4 in the 278-byte PLAY_SECONDARY_MOVIE data.  Each entry is
//   0x42 bytes; the dest rect is at entry+0x12 (x1,y1,x2,y2 as i32 LE).
//   getMovieRect reads dest_rect from the parsed action data instead of
//   centering clips.  Extracted positions include:
//     end1: (214,154)→(535,291)    end3: (257,164)→(394,291)
//     end2: (19,93)→(440,291)      end4: (237,123)→(394,291)
//     se3x: (0,0)→(535,291)        Intro: (77,44)→(452,248)
//     TV videos: (158,6)→(380,172)
//
// ## PLAY_SECONDARY_MOVIE — post-movie inventory scan
//   ADD_INVENTORY/REMOVE_INVENTORY actions positioned after a movie in the
//   action list were never reached because the movie handler returned early.
//   Example: S1250 adds the gun (item 4) conditioned on flag 44=2, which is
//   set at movie frame 174.  After playMovie completes, the handler now
//   scans all scene actions for inventory changes whose conditions pass.
//
// ## PLAY_SECONDARY_MOVIE — no autoNav for post-movie hotspot scenes
//   In scenes like S1251 (endgame gun scene), autoNav was set to the lose
//   scene (S1253) even when clickable hotspots existed (gun → S1252 win).
//   autoNav fired at the end of loadScene before the player could click.
//   The handler now skips autoNav when post-movie navigation hotspots exist,
//   letting the player interact.
//
// ## Sound double-play prevention (movieSoundsPlayed)
//   playMovie returns a Set of sound filenames triggered during playback via
//   timed_flags.  The PLAY_DIGI_SOUND handler checks this Set to avoid
//   replaying sounds that already fired during the movie (e.g. S1251's
//   explosion sounds would play twice — once from timed_flags during the
//   movie, then again from the action loop when conditions newly pass).
//
// ────────────────────────────────────────────────────────────────────────────
