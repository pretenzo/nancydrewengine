#!/usr/bin/env python3
"""
nd1_boot_decoder.py  —  Nancy Drew: Secrets Can Kill  BOOT.bin Decoder
Decodes all 29 IFF chunks: identity, fonts, sounds, map, cursors,
hints, combination puzzle, loading quotes, screen layouts, and more.
Usage: python3 nd1_boot_decoder.py [BOOT.bin]
Outputs: nd1_boot.json  nd1_boot.txt
"""
...
def decode_bootbsum(d):
    """
    BOOTBSUM (520 bytes) — Game identity + background sprite table.
    Fields: publisher(+00), series(+1e), subtitle(+3c), flags(+5a,12b),
            savegame_id(+67), title_full(+6f), n_anims(+a3,u8),
            n_overlays(+a9,u8), num_fonts(+1d1,u8).
    Sprite table (hybrid):
      First entry at +0xad: name(10)+x(u16)+y(u16)+frame(u16)+pad = 18B
      Remaining: name-only 10-byte entries at +0xdb (stop at first non-alpha).
    """
    n_anims    = u8(d, 0xa3)
    ...
    for i in range(1, n_anims+n_overlays):
        ...
        if any(32 <= c < 127 for c in raw[:6]):
            ...
            break  # hit coord data, no more names
        off += 10
    return {
...
def decode_quot(d):
    """QUOT (5760B) — 32 dev loading-screen quotes, 180B each: text\0 + author\0."""
    qs = []
...
def decode_font(d):
    """
    FONT (8076B) — 6 fonts × 0x542B each.
    Each: id_char(u8)+name(str32)+glyph_table(78×16B).
    Glyph[g] maps to char g+0x20; fields: x1,y1,x2,y2 each u16 at stride 4.
    """
    S = 0x542; fonts = []
    for i in range(len(d)//S):
        ...
        for g in range(78):
            ...
            glyphs.append({'char':chr(cc) if 32<=cc<127 else '','ascii':cc,
                           ...
                           'w':x2-x1+1 if ok else 0,'h':y2-y1+1 if ok else 0})
        fonts.append({'index':i,'id':chr(e[0]) if 32<=e[0]<127 else '?',
                      'name':cstr(e,1,32),'glyphs':glyphs})
    return fonts

def decode_sound_entry(d, o):
    """UI sound entry: filename(10)+sound_id(u16)+loop(u16)+…+vol_l(u16)+vol_r(u16)+flags(u16)."""
...
def decode_tmod(d):
    """TMOD (66B) — Tension music: 2-byte header + 2×32B entries (DADA, THUNDER3)."""
...
def _find_screen_rect(d):
    """Scan for 639/479 pair in chunk to locate the full-screen rect."""
    for o in range(0, min(len(d)-16, 64), 2):
        if u16(d,o)==639 and u16(d,o+4)==479:
            # rect is (0,0,639,479) — find the leading zeros
...
def decode_screen_chunk(d, tag=None):
    """MENU/HELP/SET — bg_image(str10)+unk_w(u16)+…+full_rect(4×i32)."""
    return {'bg_image':cstr(d,0x00,10),'screen_w':u16(d,0x0a),
            'full_rect':_find_screen_rect(d),'raw':d.hex()}

def decode_cred(d):
    """CRED (120B) — Credits screen layout and scroll parameters."""
...
def decode_load(d):
    """LOAD (681B) — Load/Save screen image names."""
...
def decode_cd(d):
    """CD (130B) — Insert-CD screen."""
    return {'bg_image':cstr(d,0x00,10),'full_rect':list(r4i32(d,0x10)),'active_rect':list(r4i32(d,0x20))}

def decode_intr(d):
    """INTR (223B) — 5 intro resource names: INTRO, INTROPAL, HER_PAL, HER_LG, HER_SM."""
    names = []
    for i in range(5):
        n = cstr(d,i*10,10)
...
def decode_map(d):
    """
    MAP (494B) — Map screen: day/night backgrounds, location hotspots, names, pins.

    Layout:
      +00  day_bg     str10    "MAPDAY"
      +0a  night_bg   str10    "MAPNIGHT"
      +78  hs_count   u16      nav hotspot count
      +7a  hotspots   N×16B    x1(u16),pad,y1(u16),pad,x2(u16),pad,y2(u16),pad
      +ea  disp_names 4×30B    location display name strings
      +162 pin_pos    4×8B     x(u16),pad,y(u16),pad  map-pin screen coords
      +1c4 location   4×12B    scene_id(u16),pad,unk,scroll_y(u16),pad,pad
    """
    hotspots = []
    off = 0x7a
    while off+16 <= 0xea:
    ...
    # Location records: scene_id(u16) at +0, scroll_y(u16) at +6, stride 12
...
def decode_curs(d):
    """
    CURS (1364B) — 85 cursor defs × 16B: sprite_x,sprite_y,hot_x,hot_y (all i32).
    Named first 10 by function; remainder numbered.
    """
...
def decode_view(d):
    """VIEW (64B) — 4 viewport rects: primary_view, full_screen, sidebar, portrait_area."""
...
def decode_hint(d):
    """HINT (3B) — Hint count per difficulty: Junior, Senior, Master."""
    return {'junior_detective':u8(d,0),'senior_detective':u8(d,1),'master_detective':u8(d,2)}

def decode_spuz(d):
    """
    SPUZ (216B) — Combination lock answer key.
    Array of u16; sentinel values 0xFFFF/0xFFF6 = unused slot.
    Solution is the sequence of non-sentinel values.
    """
...
def decode_data(d):
    """DATA (70B) — Bitmap resource name table. Scan for printable name strings."""
    names = []
    i = 0
    while i < len(d):
        s = i
...
def report(R):
    ...
    L.append('='*W); L.append("NANCY DREW: SECRETS CAN KILL  —  BOOT.bin Decoded"); L.append('='*W)
...
if __name__ == '__main__':
    path = sys.argv[1] if len(sys.argv)>1 else './nd1_extracted/BOOT.bin'
    if not os.path.exists(path): print(f"ERROR: not found: {path}"); sys.exit(1)
    R = decode_boot(path)
    with open('./nd1_boot.json','w') as f: json.dump(R,f,indent=2)
    print("JSON → /home/claude/nd1_boot.json")
    txt = report(R)
    with open('./nd1_boot.txt','w') as f: f.write(txt)
    print("Text → /home/claude/nd1_boot.txt\n")
    print(txt)
