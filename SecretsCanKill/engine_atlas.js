...
const ND = (() => {
  ...
  // Asset config
  let FRAMES_DIR, AUDIO_DIR, SPRITES_DIR, ATLASES_DIR;
  let atlasManifest = null; // { name: { w, h, count } } — loaded from manifest.json
  const atlasFrameCache = {}; // "name:frameIdx" → canvas element (sliced from atlas)
  ...
  // Load a single frame from an atlas, slicing it into a canvas element.
  // Falls back to individual PNG if atlas is unavailable.
  async function loadFrame(assetName, frameIndex) {
    const name = assetName.toLowerCase();
    const key = `${name}:${frameIndex}`;
    if (atlasFrameCache[key]) return atlasFrameCache[key];

    const info = atlasManifest?.[name];
    ...
    // Fallback: individual PNG file
    return tryLoadImg(FRAMES_DIR, assetName, `_${String(frameIndex).padStart(3, '0')}.png`);
  }

  // Get total frame count for an asset (from manifest, or -1 if unknown)
  function getFrameCount(assetName) {
  ...
  async function renderBackground(avfName, variant = 0) {
    ...
    // Try variant frame first, then frame 0
    const img = await loadFrame(avfName, variant)
             ?? (variant > 0 ? await loadFrame(avfName, 0) : null);
  ...
  // ── Init ─────────────────────────────────────────────────────────────────
  async function init({ scenesUrl, framesDir, audioDir, spritesDir, atlasesDir, startScene, initialFlags = {} }) {
    FRAMES_DIR  = framesDir;
    AUDIO_DIR   = audioDir;
    SPRITES_DIR = spritesDir;
    ATLASES_DIR = atlasesDir || framesDir;
    ...
    // Load atlas manifest (maps asset name → { w, h, count })
    try {
      atlasManifest = await fetch(`${ATLASES_DIR}/manifest.json`).then(r => r.json());
      console.log(`ND engine: ${Object.keys(atlasManifest).length} sprite atlases loaded`);
    } catch (_) {
      console.warn('ND engine: no atlas manifest found, falling back to individual PNGs');
      atlasManifest = {};
    }
