...
const ND = (() => {
  ...
  // Asset config
  let FRAMES_DIR, AUDIO_DIR, SPRITES_DIR, ATLAS_DIR;
  let START_SCENE_ID = 'S5';
  let INITIAL_FLAGS = {};
  let atlasManifest = null;  // { prefix: { w, h, cols, count } }
  const atlasCache = {};     // prefix → Image (the full atlas sheet)
  ...
  // ── Atlas frame loading ───────────────────────────────────────────────
  // Load a full atlas image (one HTTP request per animation prefix).
  async function loadAtlas(prefix) {
    const key = prefix.toLowerCase();
    if (atlasCache[key]) return atlasCache[key];
    const info = atlasManifest?.[key];
    if (!info) return null;
    try {
      const img = await loadImg(`${ATLAS_DIR}/${key}.webp`);
      atlasCache[key] = img;
      return img;
    } catch (_) { return null; }
  }

  // Extract a single frame from an atlas as a small canvas.
  // Returns a CanvasImageSource compatible with ctx.drawImage(), or null.
  function getAtlasFrame(prefix, index) {
    const key = prefix.toLowerCase();
    const atlas = atlasCache[key];
    const info = atlasManifest?.[key];
    if (!atlas || !info || index >= info.count) return null;
  ...
  // Load atlas + extract frame — single async call replacing tryLoadImg for frames.
  async function loadFrameFromAtlas(prefix, index) {
    await loadAtlas(prefix);
    return getAtlasFrame(prefix, index);
  }

  // Get frame count for an atlas prefix (0 if not in manifest).
  function getAtlasFrameCount(prefix) {
    return atlasManifest?.[prefix.toLowerCase()]?.count ?? 0;
  }

  function isAtlasPreKeyed(prefix) {
    return !!atlasManifest?.[prefix.toLowerCase()]?.keyed;
  }

  // Unified frame loader: atlas if available, else individual file fallback.
  async function loadFrame(name, index) {
    if (atlasManifest?.[name.toLowerCase()]) {
      return await loadFrameFromAtlas(name, index);
    }
    return await tryLoadImg(FRAMES_DIR, name, `_${String(index).padStart(3, '0')}.webp`);
  }

  // ── Loading indicators ──────────────────────────────────────────────────
  // Small bar: used by playMovie / playNarrationVideo / startFidget / startConvAnimation
  let _loadBarStart = 0;
  function loadBarBegin() { _loadBarStart = performance.now(); }
  function loadBarEnd()   { _loadBarStart = 0; }
  function drawLoadingBar(current, total) {
    if (!_loadBarStart || total <= 0) return;
    if (performance.now() - _loadBarStart < 150) return;
    const pct = Math.min(current / total, 1);
    ctx.save();
    const barH = 3, barY = GAME_H - barH;
    ctx.fillStyle = 'rgba(0,0,0,0.5)';
    ctx.fillRect(0, barY, GAME_W, barH);
    ctx.fillStyle = 'rgba(255,190,0,0.8)';
    ctx.fillRect(0, barY, GAME_W * pct, barH);
    ctx.restore();
  }

  // Full loading screen: used by preloadSceneAssets before a scene renders.
  function drawLoadingScreen(current, total) {
    ...
    ctx.fillText('Loading\u2026', GAME_W / 2, GAME_H / 2 - 16);
    // Bar track
    const barW = 200, barH = 6;
    const barX = (GAME_W - barW) / 2, barY = GAME_H / 2 + 4;
    ...
    ctx.fillText(`${Math.round(pct * 100)}%`, GAME_W / 2, barY + barH + 14);
    ctx.restore();
  }

  // Prefetch a conversation scene's atlas so it loads while Nancy's audio plays.
  function prefetchConvAtlas(sceneId) {
    if (!atlasManifest || !state.animationEnabled) return;
    const s = scenes[sceneId];
    if (!s) return;
    for (const act of s.actions) {
      if (act.type === 'CONVERSATION_VIDEO' && act.node_id) {
        loadAtlas(act.node_id); // fire-and-forget
        return;
      }
    }
  }

  // Conversation-only animation cache purge: after a conversation ends, free
  // memory for question/response animations (keep greeting + farewell cached).
  let _convCachePurgeSet = null; // Set<nodeId> filled by preloadSceneAssets

  function purgeConversationCache() {
    if (!_convCachePurgeSet) return;
    for (const nodeId of _convCachePurgeSet) {
      const key = nodeId.toLowerCase();
      // Purge from atlas cache (primary path when atlases are enabled)
      delete atlasCache[key];
      // Also purge any individual frames from imgCache (fallback path)
      const prefix = `${FRAMES_DIR}/${key}_`;
      for (const k of Object.keys(imgCache)) {
        if (k.startsWith(prefix)) delete imgCache[k];
      }
    }
    _convCachePurgeSet = null;
  }
  ...
  async function playMovie(act, sceneActions) {
    ...
    for (let i = 0; i <= endFrame; i++) {
      ...
      if (img) { frames.push({ index: i, img }); drawLoadingBar(i + 1, endFrame + 1); }
      else break;
    }
    loadBarEnd();
  ...
  async function playNarrationVideo(nodeId, coords, introSound, maxFrame) {
    if (!nodeId) return;

    // Load all frames for this AVF
    const frames = [];
    const count = getAtlasFrameCount(nodeId) || (maxFrame ? maxFrame + 1 : 999);
    loadBarBegin();
    for (let i = 0; i < count; i++) {
      const img = await loadFrame(nodeId, i);
      if (img) { frames.push(img); drawLoadingBar(i + 1, count); }
      else break;
    }
    loadBarEnd();
    if (frames.length === 0) return;
  ...
  async function renderBackground(avfName, variant = 0) {
    ...
    const img = await loadFrame(avfName, variant)
             ?? await loadFrame(avfName, 0);
  ...
  async function renderNPCs(actions, variant) {
    ...
    for (const act of actions) {
      ...
      if (img) {
        const cleaned = isAtlasPreKeyed(act.asset_name)
          ? img
          : getChromaKeyFrame(img, `${act.asset_name}_000`);
  ...
  async function startFidget(assetName, frameData, yS, bgSnapshot, idleEnd, hoverStart, endFrame) {
    ...
    // Preload and chroma-key all frames (skip keying if atlas is pre-keyed)
    const frames = [];
    const preKeyed = isAtlasPreKeyed(assetName);
    const count = getAtlasFrameCount(assetName) || ((endFrame ?? 999) + 1);
    loadBarBegin();
    for (let i = 0; i < count; i++) {
      const img = await loadFrame(assetName, i);
      if (!img) break;
      frames.push(preKeyed ? img : getChromaKeyFrame(img, `${assetName}_${String(i).padStart(3, '0')}`));
      drawLoadingBar(i + 1, count);
    }
    loadBarEnd();
    if (frames.length === 0) return;
  ...
  async function startConvAnimation(nodeId, coords, maxFrame) {
    stopConvAnimation();
    if (!state.animationEnabled || !nodeId) return;

    // Load all frames (conversation frames include background — no chroma-key)
    // Use maxFrame to avoid a 404 probe for the frame after the last
    const frames = [];
    const count = getAtlasFrameCount(nodeId) || ((maxFrame ?? 999) + 1);
    loadBarBegin();
    for (let i = 0; i < count; i++) {
      const img = await loadFrame(nodeId, i);
      if (!img) break;
  ...
  function showConv(act, { skipGreeting = false } = {}) {
    ...
    if (visibleChoices.length > 0 || dynQuestions.length > 0 || isInvestigationHub) {
      ...
      visibleChoices.forEach(ch => {
        ...
        btn.onclick = async () => {
          if (ch.response_scene) prefetchConvAtlas(`S${ch.response_scene}`);
      ...
      dynQuestions.forEach(q => {
        ...
        btn.onclick = async () => {
          ...
          if (q.response_scene) prefetchConvAtlas(q.response_scene);
      ...
      prefetchConvAtlas(pendingConvNav);
    } else {
    ...
    if (state.animationEnabled && !skipGreeting && act.node_id) {
      ...
      startConvAnimation(act.node_id, act.coords, act.end_frame);
    }
  ...
  async function loadScene(sceneId, variant = 0, pushHistory = true) {
    ...
    // Precache all scene assets (pano frames, fidgets, conversations) with a
    // loading bar.  Once done, rendering draws entirely from cache.
    await preloadSceneAssets(sceneId);

    // Draw background then scene items (instant — everything is cached)
    ...
    if (autoConv) {
      ...
      if (!convText) {
        ...
        await playNarrationVideo(autoConv.node_id, autoConv.coords, autoConv.intro_sound, autoConv.end_frame);
  ...
  // Silently prefetch background atlases for all scenes reachable via
  // navigation hotspots, up to MAX_DEPTH hops.  When entering a location like
  // the school this preloads every hallway/room so navigation feels instant.
  const _NAV_HS_SET = new Set(['HOT_1FR_SCENE_CHANGE', 'HOT_1FR_EXITSCENE', 'HOT_MULTIFRAME_SCENE_CHANGE']);
  ...
  function preloadAdjacentScenes(hotspots) {
    ...
    // Seed with current hotspot targets
    const queue = [];
    for (const hs of hotspots) {
      const tid = hs.action.target_scene ? `S${hs.action.target_scene}` : null;
      if (tid && !visited.has(tid)) { visited.add(tid); queue.push({ id: tid, depth: 1 }); }
    }

    // BFS through navigation links
    while (queue.length > 0) {
      const { id, depth } = queue.shift();
      const s = scenes[id];
      if (!s) continue;
      if (s.summary?.bg_avf) prefixes.add(s.summary.bg_avf.toLowerCase());
      if (depth < MAX_DEPTH) {
        for (const act of s.actions) {
          if (!_NAV_HS_SET.has(act.type) || !act.target_scene) continue;
          const next = `S${act.target_scene}`;
          if (!visited.has(next)) { visited.add(next); queue.push({ id: next, depth: depth + 1 }); }
        }
      }
    }

    // Fire-and-forget prefetch for all collected background atlases
    for (const p of prefixes) {
      if (atlasManifest?.[p] && !atlasCache[p]) {
        loadAtlas(p).catch(() => {});
      }
    }
  }
  ...
  // Collect every image URL a scene will need (background pano frames, NPC
  // fidgets, conversation animations, investigation question responses) and
  // load them all with a progress bar BEFORE the scene renders.  Once done
  // everything draws from cache — zero loading stalls during gameplay.
  // Assets stay in imgCache after the player leaves, so returning is instant.

  function collectConvNodeIds(sceneId, nodeIds) {
    const scene = scenes[sceneId];
    if (!scene) return;
    for (const act of scene.actions) {
      if (act.type !== 'CONVERSATION_VIDEO' || !act.node_id) continue;
      if (!npcRefsPass(act)) continue;
      nodeIds.add(act.node_id);
      // Follow choice branches one level deep
      if (act.choices) {
        for (const ch of act.choices) {
          if (!ch.response_scene) continue;
          const rs = scenes[`S${ch.response_scene}`];
          if (!rs) continue;
          for (const ra of rs.actions) {
            if (ra.type === 'CONVERSATION_VIDEO' && ra.node_id) nodeIds.add(ra.node_id);
          }
        }
      }
    }
  ...
  async function preloadSceneAssets(sceneId) {
    ...
    // Collect atlas prefixes to load (one HTTP request each) and extra URLs
    const atlasPrefixes = new Set();
    const extraUrls = new Set();

    // 1) Background pano
    if (sum.bg_avf) atlasPrefixes.add(sum.bg_avf.toLowerCase());

    // Animation assets — skip when animations are disabled
    if (state.animationEnabled) {
      // 2) NPC fidget frames (PLAY_SECONDARY_VIDEO)
      for (const act of scene.actions) {
        if (act.type !== 'PLAY_SECONDARY_VIDEO') continue;
        if (!condPass(act) || !act.asset_name) continue;
        atlasPrefixes.add(act.asset_name.toLowerCase());
      }

      // 3) Secondary movies (PLAY_SECONDARY_MOVIE)
      for (const act of scene.actions) {
        if (act.type !== 'PLAY_SECONDARY_MOVIE') continue;
        if (!condPass(act) || !act.asset_name) continue;
        atlasPrefixes.add(act.asset_name.toLowerCase());
      }

      // 4) Conversation animations — every question for this NPC
      const nodeIds = new Set();
      const convOnlyNodeIds = new Set();
      collectConvNodeIds(sceneId, nodeIds);

      const hubPrefix = INVESTIGATION_HUB_SCENES[sceneId];
      if (hubPrefix) {
        const allQuestions = INVESTIGATION_QUESTIONS[hubPrefix] || [];
        for (const q of allQuestions) {
          if (q.response_scene) collectConvNodeIds(q.response_scene, nodeIds);
        }
        const fw = FAREWELL_DATA[hubPrefix];
        if (fw) {
          for (const fwScene of fw.scenes) collectConvNodeIds(fwScene, nodeIds);
        }
      }

      // Track conversation-only nodes for post-conversation purge
      const greetingIds = new Set();
      for (const act of scene.actions) {
        if (act.type === 'CONVERSATION_VIDEO' && act.node_id && npcRefsPass(act)) {
          greetingIds.add(act.node_id.toLowerCase());
        }
      }
      const farewellIds = new Set();
      if (hubPrefix) {
        const fw = FAREWELL_DATA[hubPrefix];
        if (fw) {
          for (const fwScene of fw.scenes) {
            const fws = scenes[fwScene];
            if (!fws) continue;
            for (const act of fws.actions) {
              if (act.type === 'CONVERSATION_VIDEO' && act.node_id) farewellIds.add(act.node_id.toLowerCase());
            }
          }
        }
      }
      ...
      for (const nid of nodeIds) {
        atlasPrefixes.add(nid.toLowerCase());
        const low = nid.toLowerCase();
        if (!greetingIds.has(low) && !farewellIds.has(low)) convOnlyNodeIds.add(low);
      }
      if (convOnlyNodeIds.size > 0) _convCachePurgeSet = convOnlyNodeIds;
    }

    // 5) Sprite sheets (SHOW_INVENTORY_ITEM) — not atlased, load as URLs
    for (const act of scene.actions) {
      if (act.type !== 'SHOW_INVENTORY_ITEM') continue;
      if (!condPass(act) || !act.sprite_sheet) continue;
      extraUrls.add(`${SPRITES_DIR}/${act.sprite_sheet.toLowerCase()}.png`);
    }

    // Filter to uncached atlases and URLs
    const uncachedAtlases = [...atlasPrefixes].filter(p =>
      atlasManifest?.[p] ? !atlasCache[p] : !imgCache[`${FRAMES_DIR}/${p}_000.webp`]);
    const uncachedUrls = [...extraUrls].filter(u => !imgCache[u]);
    const total = uncachedAtlases.length + uncachedUrls.length;

    if (total === 0) return; // all cached

    let loaded = 0;
    drawLoadingScreen(0, total);

    // Load atlases in parallel batches
    const BATCH = 6;
    for (let i = 0; i < uncachedAtlases.length; i += BATCH) {
      const batch = uncachedAtlases.slice(i, i + BATCH);
      await Promise.all(batch.map(p => {
        if (atlasManifest?.[p]) {
          return loadAtlas(p).catch(() => {});
        }
        // Fallback: no atlas, load frame 0
        return tryLoadImg(FRAMES_DIR, p, '_000.webp').catch(() => {});
      }));
      loaded += batch.length;
      drawLoadingScreen(loaded, total);
    }

    // Load extra URLs (sprites)
    for (const u of uncachedUrls) {
      await loadImg(u).catch(() => {});
      loaded++;
      drawLoadingScreen(loaded, total);
    }
  }
  ...
  // ── Init ─────────────────────────────────────────────────────────────────
  async function init({ scenesUrl, framesDir, audioDir, spritesDir, atlasDir, startScene, initialFlags = {} }) {
    FRAMES_DIR  = framesDir;
    AUDIO_DIR   = audioDir;
    SPRITES_DIR = spritesDir;
    ATLAS_DIR   = atlasDir || null;
    ...
    // Load atlas manifest if atlas dir is configured
    if (ATLAS_DIR) {
      try {
        atlasManifest = await fetch(`${ATLAS_DIR}/manifest.json`).then(r => r.json());
        console.log(`Atlas manifest: ${Object.keys(atlasManifest).length} entries`);
      } catch (e) {
        console.warn('Atlas manifest not found, falling back to individual frames');
        atlasManifest = null;
      }
    }
